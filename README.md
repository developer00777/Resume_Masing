...
| File | Purpose |
|------|---------|
| `app/jobs.py` | Redis-backed async batch queue. FIFO backlog (`mask:queue`), a global concurrency gate capped at `MASK_MAX_CONCURRENT` (default 10) as a crash-safe lease ZSET, and per-job progress. Drives `POST /mask/batch/async` + `GET /mask/jobs/{id}`. Inert without `REDIS_URL`. |
